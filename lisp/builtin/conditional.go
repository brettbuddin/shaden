...
func andFn(env *lisp.Environment, args lisp.List) (any, error) {
	...
	return env.TailEval(args[arity-1]), nil
}
...
func ifFn(env *lisp.Environment, args lisp.List) (any, error) {
	...
	if v == nil {
		return env.TailEval(args[2]), nil
	}
	condition, ok := v.(bool)
	if !ok {
		return env.TailEval(args[1]), nil
	}
	if condition {
		return env.TailEval(args[1]), nil
	}
...
func condFn(env *lisp.Environment, args lisp.List) (any, error) {
	...
	for _, n := range args {
		if list, ok := n.(lisp.List); ok {
			...
			return env.TailEval(list[1]), nil
		}
	}
	return nil, nil
}
