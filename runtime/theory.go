...
func pitchFn(args lisp.List) (interface{}, error) {
	if len(args) != 1 {
		return nil, fmt.Errorf("pitch expects one argument")
	}
	str, ok := args[0].(string)
	if !ok {
		return nil, fmt.Errorf("pitch expects a string for argument 1")
	}
...
func intervalFn(args lisp.List) (interface{}, error) {
	if len(args) != 2 {
		return nil, fmt.Errorf("interval expects two arguments")
	}
	str, ok := args[0].(string)
	if !ok {
		return nil, fmt.Errorf("interval expects a string for argument 1")
	}
	step, ok := args[1].(int)
	if !ok {
		return nil, fmt.Errorf("interval expects an integer for argument 2")
	}
	...
	default:
		return nil, fmt.Errorf("unknown interval %s", str)
	}
}

func transposeFn(args lisp.List) (interface{}, error) {
	if len(args) != 2 {
		return nil, fmt.Errorf("transpose expects two arguments")
	}
	pitch, ok := args[0].(*musictheory.Pitch)
	if !ok {
		return nil, fmt.Errorf("transpose expects a pitch for argument 1")
	}
	interval, ok := args[1].(musictheory.Interval)
	if !ok {
		return nil, fmt.Errorf("interval expects an interval for argument 2")
	}
